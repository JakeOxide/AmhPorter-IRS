	...
	public class PatternDetectionEngine
	{
		...
		private void DetectPrefixPatterns()
        {
            ...
            for (int i = 0; i < input.Count; i++)
            {
				...
				else if (input[i].word.Length <= 3 && input[i].word[0] == 'በ' && input[i].word[1] == 'የ')
				{
        ...
        private void DetectSuffixPatterns()
		{
            ...
            for (int i = 0; i < input.Count; i++)
            {
                ...
                // Rule 318: gno-ች* -> Rem(P('የ')) - Not for words with Prefix + a single letter
                else if (input[i].word.Length >= 3 &&
                    matrixHandler.CheckFamily(input[i].word[input[i].word.Length - 2], 'ኞ') &&
                    matrixHandler.CheckFamily(input[i].word[input[i].word.Length - 1], 'ች')
                    )
                {
                    if (input[i].word.Length == 2) return;
                    input[i].wordRules.Rule318 = true;
                    input[i].wordRules.Rule319 = true;
                }
