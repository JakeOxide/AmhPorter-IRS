...
namespace AmhPorterTest.Utils
{
    public class Stemmer
    {
        ...
        private List<CustomWord> docWords;

        private List<CustomWord> originalWords;

        private List<CorpusDocument> corpusDocuments;
        ...
        public Stemmer(List<CorpusDocument> inputDocs)
        {
            corpusDocuments = inputDocs;
        }

        public List<CorpusDocument> StemWords()
        {
            inputWords = new List<CustomWord>();
            foreach (var doc in corpusDocuments)
            {
                inputWords = doc.DocumentTokens;
                patternDetectionEngine = new PatternDetectionEngine(inputWords);
                //Overwrite operation - processing
                inputWords = patternDetectionEngine.DetectPatterns();
                ExecuteTransformations();
                // Overwrite operation - result
                doc.DocumentTokens = inputWords;
            }
        ...
        private void ExecuteTransformations()
        {
            ...
            for (int i = 0; i < inputWords.Count; i++)
            {
                ...
                else if (inputWords[i].wordRules.Rule318 && inputWords[i].wordRules.Rule319)
                {
                    SubstituteCustomWord(inputWords, i, Trigger_Rule318(inputWords.ElementAt(i)));
                }
   ...
   */
        ...
        private CustomWord Trigger_Rule109(CustomWord word)
        {
            dictionary = matrix.GetDictionary();
            List<char> tempWord = word.word.ToList();
            if (tempWord.Count < 2) { word.wordRules.Rule109 = false; return word; };
            tempWord.RemoveRange(0, 2);
            CustomWord newWord = new CustomWord(new string(tempWord.ToArray()), word.wordRules);
            word.wordRules.Rule109 = false;
            return newWord;
        }
        ...
        private CustomWord Trigger_Rule318(CustomWord word)
        {
            dictionary = matrix.GetDictionary();
            List<char> tempWord = word.word.ToList();
            char key = matrix.FindKey(word.word[word.word.Length - 2]).Item2;
